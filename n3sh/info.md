...
| **pqrs 등록** | 🚧 **제출 예정** (2026-08-10 전환) — 산출물 착수 전. 선행· 이후 |
...
>
> 📌 **pqrs 전환 (2026-08-10)**: 미제출 사유는 **받아 줄 문서가 없었다**는 것이다(사용자 진술로 처음 기록). 254 규칙·세벌식 390 전제를 등록 페이지 한 칸에 담을 수 없어, 등록해도 쓰는 사람이 조작법을 알 길이 없었다. 로 공개 저장소·문서 트리가 서면서 그 제약이 사라졌다. 종전 표에 적혀 있던 *"규칙 1건 단위 체계에 맞지 않는다"* 는 **지어낸 사유**였다 — 이 규칙도 `rules` 1건(manipulator 227개)으로 등록된 FootPedal 과 같은 형태다.
