...
> 🌐 **pqrs 공개 등록** (PR #2001 · 2026-08-30) — [커뮤니티 카탈로그](https://ke-complex-modifications.pqrs.org/?q=n3sh) 에 올라가 있다.
> 규칙 파일 직접 주소는 <https://ke-complex-modifications.pqrs.org/json/nowage_n3sh_390.json> 다.
...
라이브 rule 이름은 `n3sh(Nowage ShortHand for 3set 390) : {저장소 URL} v{버전}` 꼴이다 — 버전은 **[VERSION](VERSION)**(이 폴더) 이 SSOT 이고 `emit.rule_description` 이 각인한다(옮겨 적은 값은 낡으므로 그 파일을 본다). 2026-09-01 부터 날짜가 아니라 숫자(semver)다.
