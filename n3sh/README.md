...
세벌식 390 자판에서 **자모 2개를 동시에 누르면 완성형 한 글자가 한 번에 나온다.** 세벌식은 초성·중성·종성을 따로 치므로 받침 있는 글자가 3타인데, 자주 나오는 글자를 2타로 줄인다. 254 규칙이 들어 있다.
...
| **pqrs 등록 (390)** | 🚧 **제출 예정** — 2026-08-10 전환. 산출물 착수 전이며 선행 2건([FootPedal 갱신](../footPedal/README.md)·[EngCharOnKor 등록](../engCharOnKor/README.md)) 뒤에 간다 |
| **pqrs 등록 (최종식)** | ⏸️ **요청이 있을 때만** — 390 제출 후, 쓰겠다는 사람이 나타나면 그때 **별도 프로젝트로 독립시켜** 공개한다. 요청 없이 먼저 내지 않는다 |
...
* **왜 이제 내는가** — 로 공개 저장소와 문서 트리가 섰다. 자판·학습 단계·규칙표를 GitHub 문서로 낼 수 있으므로 `extra_descriptions` 는 요약과 링크만 담으면 된다
